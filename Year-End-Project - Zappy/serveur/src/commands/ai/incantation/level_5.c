...
int check_inventory_level_5(client_t *client, info_game_t *info)
{
    int check = 0;
...
void do_incantation_level_5(client_t *client, info_game_t *info)
{
...
{
    list_client_t *choosen = get_list_client_same_level(&info->list_client, 4,
    ...
    check = check_inventory_level_5(client, info);
    if (check >= 4)
        do_incantation_level_5(client, info);
    else
        send_end_incantation(info, client, "ko");
}
